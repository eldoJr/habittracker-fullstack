...
CREATE TABLE habits (
    ...
    frequency_config JSONB NOT NULL, -- {days: [1,3,5], times_per_week: 3}
    color VARCHAR(7) DEFAULT '#6366f1',
    icon VARCHAR(50) DEFAULT 'target',
    target_duration INTEGER, -- in minutes (for timed habits)
    reminder_time TIME[],
...
CREATE TABLE habit_completions (
    ...
    duration INTEGER, -- actual time spent in minutes
    notes TEXT,
...
-- Schedule monthly partition creation (runs daily at midnight)
...
-- Performance indexes
CREATE INDEX CONCURRENTLY idx_completions_lookup 
    ON habit_completions(habit_id, completed_date DESC);

CREATE INDEX CONCURRENTLY idx_completions_user 
...
BEGIN
    SELECT COALESCE(current_streak, 0) INTO streak_count
    FROM user_streaks
...
BEGIN
    SELECT 
        ROUND((COUNT(*)::NUMERIC / days) * 100, 2)
    INTO completion_rate
    FROM habit_completions
    WHERE habit_id = p_habit_id
        AND completed_date >= CURRENT_DATE - days;
